...
import { Board, Square, Piece, Move, PieceName, PromotionMove } from '@/lib/types/main'
...
const useChessboard = create<BoardState>((set, get) => ({
    ...
    handlePromotion: (promotionTo: PieceName) => {
        ...
        set((state) => ({ ...movePiece(state, promotionMove), selectedPiece: null, selectedSquare: null, promotionSquare: null }))
    }
}))
